...
export const HadithCard = React.memo(({ todayHadith }: HadithCardProps) => {
  const [expandedSection, setExpandedSection] = useState<'explanation' | 'benefits' | 'action' | null>(null);
  ...
  return (
    <div className="w-full flex-shrink-0 snap-center px-2 flex flex-col justify-start pt-2">
      <div className="relative overflow-hidden rounded-[28px] border border-white/60 bg-white/60 backdrop-blur-2xl p-5 shadow-[0_8px_32px_rgba(43,26,16,0.06)] flex flex-col max-h-[85vh] md:max-h-full">
        {/* Header */}
        <div className="mb-3 flex items-center gap-3 shrink-0">
          ...
          <div>
            <p className="text-[13px] font-black text-[#2b1a10]">حديث اليوم</p>
        ...
        <div className="overflow-y-auto no-scrollbar pb-2">
          ...
          <div className="mt-3 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2">
              ...
              <p className="text-[10px] font-bold text-[#b88a4f]">{todayHadith.source}</p>
            </div>
          </div>
          ...
          <div className="mt-4 flex flex-col gap-2">
            ...
            <div className="overflow-hidden rounded-xl border border-[#e6dccf]/60 bg-white/40">
              ...
              >
                <div className="flex items-center gap-2">
                  <BookOpenText className="h-4 w-4 text-[#b88a4f]" strokeWidth={2.5} />
                  <span className="text-[12px] font-bold text-[#2b1a10]">شرح الحديث</span>
                </div>
                <motion.div
                  animate={{ rotate: expandedSection === 'explanation' ? 180 : 0 }}
            ...
            <div className="overflow-hidden rounded-xl border border-[#e6dccf]/60 bg-white/40">
              ...
              >
                <div className="flex items-center gap-2">
                  <Sparkles className="h-4 w-4 text-[#b88a4f]" strokeWidth={2.5} />
                  <span className="text-[12px] font-bold text-[#2b1a10]">فوائد مستنبطة</span>
                </div>
                <motion.div
                    ...
                    <div className="px-3 pb-3 pt-1 border-t border-[#e6dccf]/30">
                      <ul className="list-disc pr-4 space-y-1">
                        {todayHadith.benefits.map((benefit, index) => (
                          <li key={index} className="text-[12px] leading-relaxed text-[#4a3b32]">
                            {benefit}
                          </li>
                        ))}
                      </ul>
            ...
            <div className="overflow-hidden rounded-xl border border-[#e6dccf]/60 bg-white/40">
              ...
              >
                <div className="flex items-center gap-2">
                  <Lightbulb className="h-4 w-4 text-[#deab65]" strokeWidth={2.5} />
                  <span className="text-[12px] font-bold text-[#2b1a10]">كيف تعمل به اليوم؟</span>
                </div>
                <motion.div
                      ...
                      <ul className="space-y-2">
                        {todayHadith.practicalAction.map((action, index) => (
                          <li key={index} className="flex gap-2">
                            <span className="text-[#deab65] mt-0.5 shrink-0">✦</span>
